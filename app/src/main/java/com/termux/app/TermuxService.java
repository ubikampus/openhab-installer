...
public final class TermuxService extends Service implements SessionChangedCallback {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_STOP_SERVICE.equals(action)) {
            ...
            if (intent.getBooleanExtra(EXTRA_EXECUTE_IN_BACKGROUND, false)) {
                ...
                TerminalSession newSession = createTermSession(executablePath, arguments, cwd);
    ...
    TerminalSession createTermSession(String executablePath, String[] arguments, String cwd) {
        new File(HOME_PATH).mkdirs();

        if (cwd == null) cwd = HOME_PATH;

        String[] env = BackgroundJob.buildEnvironment(cwd);
